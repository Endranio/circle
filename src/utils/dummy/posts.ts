import { Post } from '@/features/home/types/posts';

export const postDatas: Post[] = [
  {
    id: 'op-luffy-12345',
    user: {
      fullname: 'Monkey D. Luffy',
      username: 'luffy_gear5',
      avatarUrl:
        'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Monkey%20D.%20Luffy',
    },
    content: 'I will become the Pirate King!',
    likesCount: 5000,
    repliesCount: 3,
    replies: [
      {
        id: 'op-zoro-67890',
        user: {
          fullname: 'Roronoa Zoro',
          username: 'zoro_santoryu',
          avatarUrl:
            'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Roronoa%20Zoro',
        },
        content: 'Do not get lost at sea, Captain!',
        likesCount: 1200,
        createdAt: new Date(),
      },
      {
        id: 'op-sanji-54321',
        user: {
          fullname: 'Sanji Vinsmoke',
          username: 'sanji_blackleg',
          avatarUrl:
            'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Sanji%20Vinsmoke',
        },
        content: 'Do you need more meat, Luffy?',
        likesCount: 900,
        createdAt: new Date(),
      },
      {
        id: 'op-shanks-11223',
        user: {
          fullname: 'Shanks',
          username: 'shanks_redhair',
          avatarUrl:
            'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Shanks',
        },
        content: 'I have high hopes for you, Luffy!',
        likesCount: 5000,
        createdAt: new Date(),
      },
    ],
    createdAt: new Date(),
  },
  {
    id: 'op-roger-98765',
    user: {
      fullname: 'Gol D. Roger',
      username: 'roger_pirateking',
      avatarUrl:
        'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Gol%20D.%20Roger',
    },
    content: 'The greatest treasure is at the end of the Grand Line. Find it!',
    likesCount: 9999,
    repliesCount: 2,
    replies: [
      {
        id: 'op-rayleigh-22446',
        user: {
          fullname: 'Silvers Rayleigh',
          username: 'rayleigh_darkking',
          avatarUrl:
            'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Silvers%20Rayleigh',
        },
        content: 'Those words changed the world, Captain.',
        likesCount: 4500,
        createdAt: new Date(),
      },
      {
        id: 'op-whitebeard-99887',
        user: {
          fullname: 'Edward Newgate',
          username: 'whitebeard_yonko',
          avatarUrl:
            'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Edward%20Newgate',
        },
        content: 'ONE PIECE IS REAL!!!',
        likesCount: 8000,
        createdAt: new Date(),
      },
    ],
    createdAt: new Date(),
  },
  {
    id: 'op-robin-33445',
    user: {
      fullname: 'Nico Robin',
      username: 'robin_poneglyph',
      avatarUrl:
        'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Nico%20Robin',
    },
    content: 'I just want to know the history of the world...',
    likesCount: 3000,
    repliesCount: 1,
    replies: [
      {
        id: 'op-franky-66778',
        user: {
          fullname: 'Franky',
          username: 'franky_shipwright',
          avatarUrl:
            'https://api.dicebear.com/9.x/notionists-neutral/svg?seed=Franky',
        },
        content: 'SUPER! I will help you build your dream!',
        likesCount: 1800,
        createdAt: new Date(),
      },
    ],
    createdAt: new Date(),
  },
];
